...
export default function SettingsPage() {
  ...
  useEffect(() => {
    apiFetch<Settings>("/api/settings").then(setSettings).catch(() => undefined);
  }, []);
    ...
    <Card>
      <div className="header-row">
        <div>
          ...
          <p className="table-subcopy">Company data, pay rules, tax rates, leave policy, and face matching thresholds.</p>
        </div>
      ...
      <div className="form-grid" style={{ marginTop: 16 }}>
        ...
        <div className="grid-three">
          <Field label="Income Tax Rate">
            <input className="text-input" type="number" step="0.01" value={settings.income_tax_rate} onChange={(event) => setSettings({ ...settings, income_tax_rate: Number(event.target.value) })} />
          </Field>
